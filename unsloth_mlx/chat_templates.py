...
from typing import Any, Dict, List, Optional, Callable, Union, NamedTuple, Tuple, Iterable
...
__all__ = [
    ...
    # Template registry
    # JSONL conversion utilities (GUI)
    'infer_sft_messages',
    'convert_rows_to_messages_jsonl',
    'convert_dataset_to_messages_jsonl',
    'CHAT_TEMPLATES',
    'TEMPLATE_ALIASES',
...
# =============================================================================
# JSONL CONVERSION UTILITIES (for GUI)
# =============================================================================
...
def infer_sft_messages(obj: Dict[str, Any]) -> Tuple[Optional[List[Dict[str, str]]], str]:
    """Infer SFT messages from various dataset formats.
    
    Returns (messages, source_format) where source_format is one of:
    - 'messages', 'conversations', 'alpaca', 'prompt_completion', 'unrecognized'
    """
    # Use normalize_messages from this module
    ...
    if "conversations" in obj:
        ...
        if isinstance(conv, list) and len(conv) > 0:
            ...
            for turn in conv:
                ...
                role = role_map.get(role.lower() if isinstance(role, str) else role, role)
    ...
    if "instruction" in obj and "output" in obj:
        ...
        if instruction and output:
            ...
            msgs = []
            if obj.get("system"):
                msgs.append({"role": "system", "content": str(obj["system"]).strip()})
    ...
    completion = obj.get("completion") or obj.get("response")
    if prompt and completion:
        msgs = []
        if obj.get("system"):
            msgs.append({"role": "system", "content": str(obj["system"]).strip()})
        msgs.append({"role": "user", "content": str(prompt).strip()})
        msgs.append({"role": "assistant", "content": str(completion).strip()})
...
def convert_rows_to_messages_jsonl(
    ...
    """Convert rows from various formats to messages JSONL."""
...
def convert_dataset_to_messages_jsonl(
    ...
    """Convert a dataset to messages JSONL format."""
    import time
    ts = int(time.time())
